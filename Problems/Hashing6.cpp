 /*
   ...
   Problem Name   :  Implementing the Linear Probing.
*/
		#include<bits/stdc++.h>
		using namespace std;
        ...
        void linearProbing(int *hash,int h,int *arr,int n)
        {
                    //Your code here
                    int p;
                    for(int i=0;i<n;i++)
                    {
                            p=0;
                            //cout<<"ram"<<"\n";
                            while(hash[((arr[i] % h) + p) % h] != -1 && is_vacant(hash,h))
                            {
                                p++;
                            }
                            if(is_vacant(hash,h))
                            hash[((arr[i] % h) + p) % h]=arr[i];
                            else
                            {
                                continue;
                            }
		...
		int main()
		{
				    ...
				    while(t--)
				    {
                       ...
                       linearProbing(hash_table,n_hash_size,arr,n_array_size);
                        cout<<"\n";
				    }
				    return 0;
		}